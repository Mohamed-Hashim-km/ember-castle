...
export const Hero: React.FC<HeroProps> = ({
  ...
  const getOptimizedVideoSrc = (url?: string) => {
  ...
  const finalPosterSrc = posterSrc || getPosterSrc(videoSrc);

  return (
    <>
      {/* DESKTOP VERSION */}
      <section className="hidden md:flex relative w-full h-[90vh] flex-col justify-center items-center pt-20">
        <div className="absolute inset-0 z-0 ">
          <div className="absolute inset-0 bg-black/30 z-10" />
          {finalPosterSrc && (
            <Image src={finalPosterSrc} alt={title || "Hero background"} fill className="object-cover" priority />
          )}
          {optimizedVideoSrc ? (
            <video
              autoPlay
            ...
            !finalPosterSrc && <Image src="/1.webp" alt="Luxury Bedroom" fill className="object-cover" priority />
          )}
        </div>
...
export const BookingBar = () => {
  ...
  // Form valid check (removed location condition)
  const isFormValid = dateRange.start && dateRange.end && adults > 0;
      ...
      <div className="hidden lg:flex relative w-auto mx-auto bg-[#FEFEFE] py-6 px-6 flex-row items-center gap-16 rounded-sm transition-all duration-300">
        ...
        <div className="">
          <Button
            ...
            onClick={() => {
              openModal({
                location: "Gokarna", // Hardcoded Location
